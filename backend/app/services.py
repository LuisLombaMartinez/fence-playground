...
def get_assets() -> list[Asset]:
    random.seed(42)
    statuses: list[Literal["active", "defaulted", "paid"]] = [
        "active",
        "defaulted",
        "paid",
    ]
...
def calculate_insights(assets: list[Asset]) -> list[Insight]:
    ...
    return [
        Insight(
            id="insight-001", name="total_portfolio_value", value=round(total_value, 2)
        ),
        Insight(id="insight-002", name="default_rate", value=round(default_rate, 4)),
        Insight(
            id="insight-003", name="outstanding_debt", value=round(outstanding_debt, 2)
        ),
        Insight(
            id="insight-004", name="collection_rate", value=round(collection_rate, 4)
        ),
    ]
